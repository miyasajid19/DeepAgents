...
from deepagents.backends import StateBackend, CompositeBackend, StoreBackend,LocalShellBackend
...
backend = CompositeBackend(
    default=LocalShellBackend( root_dir=".", virtual_mode=True, env={ "PATH": "/usr/bin:/bin" } ),
    routes={
        "/memories":StoreBackend( namespace=lambda rt: ( rt.server_info.user.identity if rt.server_info and rt.server_info.user else "default-user", ), ),
        "files": StateBackend()
    }
)
...
print("=" * 65)
print("Deep Agents + CompositeBackend + InMemorySaver")
print("=" * 65)
